...
@Component
public class BlockFileReceiver extends Thread {
	 ...
	 */
	private static Map<String, Map<GetStatusRequest, String>> mapHostStatus = new ConcurrentHashMap<>();
	...
	public void run() {
		...
		try {
			serverSocket = new ServerSocket(ConfigConstants.PORT_FOR_BLOCK_FILE);
			while (true) {
				try {
					final Socket socket;
					try {
					...
					final Runnable listener = () -> {
						try (DataInputStream dataIn = new DataInputStream(
							...
							// ois.readObject();
							final String jsonString = "" + dataIn.readUTF();
							...
							// } else if
							// (receivedBlockAndFileModel.getMapWithStatus() !=
							// null) {
							// logger.info("Message Received: " +
							// receivedBlockAndFileModel.toString()
							// + " from the Host: " +
							// socket.getInetAddress().getHostAddress());
							// final Map<GetStatusRequest, String> mapWithStatus
							// = new ConcurrentHashMap<>();
							// mapWithStatus.putAll(receivedBlockAndFileModel.getMapWithStatus());
							// mapHostStatus.put(socket.getInetAddress().getHostAddress(),
							// mapWithStatus);
							// } else if (receivedBlockAndFileModel.getBlock()
							// != null) {
							// // final byte[] file =
							// receivedBlockAndFileModel.getFile();
							if (receivedBlockAndFileModel != null && receivedBlockAndFileModel.getBlock() != null) {
